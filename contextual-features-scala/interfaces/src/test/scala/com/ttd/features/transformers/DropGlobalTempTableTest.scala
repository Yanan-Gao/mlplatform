...
class DropGlobalTempTableTest extends TTDSparkTest with DatasetComparer {
  test("Serializable drop table stage") {
    ...
    TempPathUtils.withTempPath(path => {
      val pipe = new Pipeline().setStages(Array(
        // need to create one to be able to drop it
        CreateGlobalTempTable("tempTest"),
        DropGlobalTempTable("tempTest")
      ))
