...
import com.ttd.mycellium.Edge.UserIDConnectedFromIPAddress
import com.ttd.mycellium.Vertex.{IPAddress, UserID}
import org.apache.spark.ml.Pipeline
import org.apache.spark.sql.functions._
import org.apache.spark.sql.types.{ArrayType, FloatType, IntegerType, LongType, MapType, StringType}

class ConnectedFromEdgesPipe extends Pipeline {
  setStages(Array(
    // Drop if can't associate with any IPAddress
    DropNA(Array("IPAddress")),
    Select(Seq(
      col("ts"),
      UserIDConnectedFromIPAddress.eType as "e_type",
      UserID.idColumn as "v_id1",
      IPAddress.idColumn as "v_id2"
    )),
    ...
    WithColumn("e_id", UserIDConnectedFromIPAddress.idColumn)
  ))
}
