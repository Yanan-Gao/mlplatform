...
object UserFeatureMergeJob {
  ...
  object Config {
    val UserFeatureMergeDefinitionPath = config.getString("userFeatureMergeDefinitionPath", default = "./userFeatureMergeDefinition.json")
  }
  ...
  def runETLPipeline(): Unit = {
    ...
    val definition = FSUtils.readStringFromFile(Config.UserFeatureMergeDefinitionPath)(spark)
    ...
    UserFeatureDataset(userFeatureMergeDefinition)
      .writePartition(df.select(col(FeatureConstants.UserIDKey), col(FeatureConstants.FeatureDataKey)).as[UserFeature], dateTime)
