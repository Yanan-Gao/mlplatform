...
from philo.data import create_datasets, prepare_dummy_data, prepare_real_data, s3_sync
...
flags.DEFINE_integer('num_epochs', default=10, help='Number of epochs for training', lower_bound=1)
...
def main(argv):
    ...
    datasets = prepare_dummy_data(model_features, model_target, 32) if FLAGS.dummy else prepare_real_data(
        model_features, model_target, FLAGS.input_path, FLAGS.batch_size, FLAGS.eval_batch_size)
    ...
    epoch_gauge.set(FLAGS.num_epochs)
    ...
    eval_anlp_gauge = Prometheus.define_gauge('eval', 'evaluation')
    eval_anlp_gauge.set(evals[2])
