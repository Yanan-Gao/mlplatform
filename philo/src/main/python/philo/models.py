...
def deep_fm_dual_tower(linear_feature_columns, dnn_feature_columns, adgroup_feature_list, fm_group=[DEFAULT_GROUP_NAME],
    ...
    if dnn_hidden_units:
        dual_combined_output, _, _ = create_combined_encoder(group_embedding_dict, dense_value_list,
                                                             adgroup_feature_list,
...
def deep_fm_dual_tower_neo(linear_feature_columns, dnn_feature_columns, adgroup_feature_list,
    ...
    features = build_input_features(
        linear_feature_columns + dnn_feature_columns)
    inputs_list = list(features.values())
    inputs_list_a, inputs_list_b = separate_feature(inputs_list, adgroup_feature_list=adgroup_feature_list)

    linear_feature_columns_a, linear_feature_columns_b = separate_feature(linear_feature_columns,
                                                                          adgroup_feature_list=adgroup_feature_list)
    features_a, features_b = separate_feature(features, adgroup_feature_list=adgroup_feature_list)

    linear_logit_a = get_linear_logit(features_a, linear_feature_columns_a, seed=seed, prefix='linear',
                                      l2_reg=l2_reg_linear)
    linear_logit_b = get_linear_logit(features_b, linear_feature_columns_b, seed=seed, prefix='linear',
                                      l2_reg=l2_reg_linear)
    linear_logit = linear_logit_a + linear_logit_b
    ...
    # Deep part of the model, which is factorization operations
    # build the adgroup and bidrequest FM parts separately for Neo model output,
    # and then combine together to get the original FM layer logit
    embed_logits_a, embed_logits_b = separate_feature(group_embedding_dict[fm_group[0]],
                                                      adgroup_feature_list=adgroup_feature_list)
    fm_logits_a, fm_logits_b = get_partial_fm(embed_logits_a), get_partial_fm(embed_logits_b)
    fm_logit_combined = combine_neo_fm_logits(fm_logits_a, fm_logits_b)

    # deep fm use both embedded feature and original dense features in the deep tower
    # dnn_input = combined_dnn_input(list(chain.from_iterable(
    #     group_embedding_dict.values())), dense_value_list)
    if dnn_hidden_units and len(dnn_hidden_units) > 0:
        dnn_logit, dnn_output_a, dnn_output_b = create_combined_encoder(group_embedding_dict, dense_value_list,
                                                                        adgroup_feature_list,
        ...
        raise Exception("dnn_hidden_units cannot be null or empty.")
        # if no list, then directly connect to a dense output, which will be equal to a linear model based on embedding
        # dual_combined_output = Dense(1, use_bias=False, kernel_initializer=glorot_normal(seed=seed))(dnn_input)
