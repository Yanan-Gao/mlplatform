...
Feature = namedtuple("Feature",
                     "name, sparse, type, cardinality, default_value, enabled, embedding_dim, qr_embed, qr_collisions, max_len")
...
def get_features_from_json(json_path, exclude_features=[], default_emb_dim=DEFAULT_EMB_DIM, get_adgroup_feature=False):
    ...
    for f in features:
        if f["Name"] not in exclude_features:
            ...
            max_len = f["MaxLength"] if "MaxLength" in f else 0
            ...
            feature_list.append(Feature(f["Name"], sparse, tf_type, cardinality, default_value, True, emb_dim, qr_embed, qr_collisions, max_len))
