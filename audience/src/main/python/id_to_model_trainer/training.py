...
flags.DEFINE_string('model_creation_date', default=datetime.now().strftime("%Y%m%d"),
...
flags.DEFINE_string('model_weight_path', default=None, help="Path to existing model weights. The model will not be restored if this argument is not supplied.")
flags.DEFINE_boolean('upload_weights', default=False, help='Upload weights for saved model.')

# Model Choices
flags.DEFINE_integer("search_embedding_size", default=64, help="Embedding size for impression and pixel embedding. Should match the length defined in the similarity search engine")

# Train params
flags.DEFINE_integer('batch_size', default=10240, help='Batch size for training', lower_bound=1)
flags.DEFINE_integer('num_epochs', default=12, help='Number of epochs for training', lower_bound=1)
flags.DEFINE_integer('buffer_size', default=1000000, help='Buffer size for training', lower_bound=1)

# Eval params
flags.DEFINE_integer('eval_batch_size', default=102400, help='Batch size for evaluation')

# Call back
flags.DEFINE_boolean("save_best", default=False, help="if save best is set to false, the model will exit early when the weights have not been changed for `early_stopping_patient` epoches.")
...
def get_data(parser):
    ...
    train = data.tfrecord_dataset(train_files, FLAGS.batch_size, parser.parser(), train=True, buffer_size=FLAGS.buffer_size)
    val = data.tfrecord_dataset(val_files, FLAGS.eval_batch_size, parser.parser(), train=False)

    return train, val


def get_callbacks(save_best=True):
    es = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=FLAGS.early_stopping_patience, restore_best_weights=True)
...
def main(argv):
    ...
    model = models.init_model(features.model_features, features.model_dim_group, FLAGS.search_embedding_size)
