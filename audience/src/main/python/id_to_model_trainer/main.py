...
flags.DEFINE_string(
    "input_path",
    default=INPUT_PATH,
    help=f"Location of training input files (TFRecord). Default {INPUT_PATH}",
)
...
def main(argv):
    print(
        "Running application with configuration items: %s"
        % " ".join([f + "=" + str(getattr(FLAGS, f)) for f in FLAGS])
    )
