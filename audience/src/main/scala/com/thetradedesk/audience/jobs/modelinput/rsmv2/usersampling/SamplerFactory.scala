...
object SamplerFactory {
  ...
  def fromString(name: String, conf: Any): Sampler = {
    name match {
      ...
      case "RSMV2Measurement" => RSMV2MeasurementSampler(conf) // used by measurement job to speed up process
      case "RSMV2Population" => RSMV2PopulationSampler(conf) // used by measurement job to speed up process
