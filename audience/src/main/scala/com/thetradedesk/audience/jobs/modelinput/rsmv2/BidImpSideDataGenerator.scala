...
object BidImpSideDataGenerator {
  ...
  def readBidImpressionWithNecessary(conf: RelevanceModelInputGeneratorJobConfig): Dataset[BidSideDataRecord] = {
    ...
    val bidsImpressionsLongRaw = loadParquetData[BidsImpressionsSchema](bidImpressionsS3Path, date, lookBack = Some(0), source = Some(GERONIMO_DATA_SOURCE))
      ...
      .withColumn("IDType", when('DeviceAdvertisingId.isNotNull, lit(IDTransform.IDType.DeviceAdvertisingId.id))
        .when('CookieTDID.isNotNull, lit(IDTransform.IDType.CookieTDID.id))
        .when('UnifiedId2.isNotNull, lit(IDTransform.IDType.UnifiedId2.id))
        .when('EUID.isNotNull, lit(IDTransform.IDType.EUID.id))
        .when('IdentityLinkId.isNotNull, lit(IDTransform.IDType.IdentityLinkId.id))
        .otherwise(lit(IDTransform.IDType.Unknown.id)))
  ...
  def prepareBidImpSideFeatureDataset(rawBidReq: Dataset[BidSideDataRecord], conf: RelevanceModelInputGeneratorJobConfig): BidResult = {
    ...
    val res = rawBidReq
      ...
      .withColumn("IDType", col("AllIdsWithIdTypes").getField("_2"))
