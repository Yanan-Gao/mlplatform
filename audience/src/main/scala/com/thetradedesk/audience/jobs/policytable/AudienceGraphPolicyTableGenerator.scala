...
abstract class AudienceGraphPolicyTableGenerator(
    ...
    extends AudiencePolicyTableGenerator(model, prometheus,  confettiEnv, experimentName, conf) {
  ...
  def generateRawPolicyTable(sourceMeta: Dataset[SourceMetaRecord], date: LocalDate): DataFrame = {
    ...
    val policyTable =
      nonGraphCount
        ...
        .withColumn("StorageCloud", lit(StorageCloud.withName(conf.storageCloud).id))
        .cache()
