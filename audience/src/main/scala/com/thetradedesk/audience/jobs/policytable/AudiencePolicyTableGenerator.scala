...
abstract class AudiencePolicyTableGenerator(
    model: Model,
    val conf: AudiencePolicyTableGeneratorJobConfig,
  ...
  protected val storageCloudId: Int = StorageCloud.withName(conf.storageCloud).id
  ...
  private def updateSyntheticId(date: LocalDate, policyTable: DataFrame, previousPolicyTableRaw: Dataset[AudienceModelPolicyRecord], previousPolicyTableDate: LocalDate): DataFrame = {
    ...
    val inActiveIds = previousPolicyTable.filter('StorageCloud === storageCloudId)
    ...
    // get new sourceId
    val newIds = activeIds.join(previousPolicyTable.filter('StorageCloud === storageCloudId)
    ...
    val currentActiveIds = policyTable
      .join(previousPolicyTable.filter('StorageCloud === storageCloudId).select('SourceId, 'Source, 'CrossDeviceVendorId, 'SyntheticId, 'IsActive, 'StorageCloud, 'MappingId), Seq("SourceId", "Source", "CrossDeviceVendorId", "StorageCloud"), "inner")
    ...
    val otherSourcePreviousPolicyTable = previousPolicyTable.filter('StorageCloud =!= storageCloudId).toDF()
