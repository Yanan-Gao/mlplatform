...
class AEMPolicyTableGenerator(conf: AudiencePolicyTableGeneratorJobConfig, prometheus: PrometheusClient)
  extends AudiencePolicyTableGenerator(Model.AEM, conf, prometheus) {
...
object AEMPolicyTableGenerator {
  def generatePolicyTable(conf: AudiencePolicyTableGeneratorJobConfig): Unit =
    new AEMPolicyTableGenerator(conf, AudiencePolicyTableGeneratorJob.prometheus.get)
      .generatePolicyTable()
}
