...
final case class AudienceModelV2InputRecord(
...
case class AudienceModelV2InputDataset(model: String, tag: String, version: Int = 1) extends
  LightWritableDataset[AudienceModelV2InputRecord](s"/${ttdEnv}/audience/${model}/${tag}/v=${version}", S3Roots.ML_PLATFORM_ROOT, audienceResultCoalesce, dateFormat = audienceVersionDateFormat)
