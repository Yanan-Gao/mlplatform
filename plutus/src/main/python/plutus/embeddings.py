...
class QREmbedding(keras.layers.Layer):
    ...
    def __init__(self, 
        ...
        if self.do_hashing:
            self.hasher = keras.layers.experimental.preprocessing.Hashing(num_bins=vocab_size, dtype=tf.string, name=f"{name}_hasher")
...
def qr_embedding(column, vocab_size=15, emb_dim=5, num_collisions=4, feat_dtype=tf.int32):
    """
    ...
    if feat_dtype == tf.int32:
        em = QREmbedding(vocab_size=vocab_size, embedding_dim=emb_dim, num_collisions=num_collisions, name=f"{column}_qr_embedding")
    elif feat_dtype == tf.string:
        em = QREmbedding(vocab_size=vocab_size, embedding_dim=emb_dim, num_collisions=num_collisions, name=f"{column}_qr_embedding", do_hashing=True)
    else:
...
def int_embedding(column, vocab_size, emb_dim=5, dtype=tf.int32):
    i = keras.Input(shape=(1,), name=column, dtype=dtype)
...
def hashing_encoder(column, vocab_size=15):
    ...
    h = keras.layers.experimental.preprocessing.Hashing(num_bins=vocab_size, name=f"{column}_hasher")
    ec = keras.layers.experimental.preprocessing.CategoryEncoding(max_tokens=vocab_size, name=f"{column}_encoder", output_mode="binary")
    return i, h, ec
