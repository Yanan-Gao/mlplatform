...
/*
 * Note from Yuehan:
 * This is a direct copy from El-dorado
 */
...
trait FSUtils {
  def writeStringToFile(file: String, data: String)(implicit spark: SparkSession) : Unit
}
 ...
 * !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 */
object FSUtils extends FSUtils {
  /**
  ...
  def getLatestPathBefore(path: String, date: String, lookbackWindow: Int,
                          dateToPath: (LocalDate) => String = (d => d.toString))
                         (implicit spark: SparkSession): String = {
    ...
    while (i <= lookbackWindow && !find) {
      ...
      pathFound = s"${path}/${dateToPath(curDate)}"
