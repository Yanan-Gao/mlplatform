...
/**
 * ThreadUtils class taken froom org.apache.spark library. Required to re-implement the [[com.ttd.benchmarks.automl.TrainValidationSplitBenchmark]]
 * class with the same threading functionality to train multiple models at once. The class was re-implemented to add
 * MlFlow tracking capability which could not otherwise be injected.
 */
object ThreadUtils {
