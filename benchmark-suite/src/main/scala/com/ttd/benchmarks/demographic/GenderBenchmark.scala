...
class GenderBenchmark[M <: PredictionModel[Vector, M]]
    (estimator: Estimator[M], dates: Seq[DateTime], paramGrid: Array[ParamMap]) extends Estimator[M] {
  override val uid: String = Identifiable.randomUID("GenderBenchmark")
  ...
  override def fit(ds: Dataset[_]): M = {
    ...
    var ageLabels = $(dateRegionLabelsDS)
      ...
      .select($"userId" as $(joinCol), $"genderLabel" as "label")
    ...
    val classificationBenchmark = new Benchmark[M](estimator, paramGrid, evaluator)
    ...
    classificationBenchmark.fit(featuresAndLabels)
  }
