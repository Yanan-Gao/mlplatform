...
def get_string_mapping_layer(input, asset_root='./conversion-python/assets_string/'):
    lookup_out = VocabLookup(vocab_path=asset_root+input.name+'.txt')
    return lookup_out
...
def string_map_embedding(name, em_size, assets_path):
    i = keras.Input(shape=(1,), dtype=tf.string, name=f"{name}")
    l = get_string_mapping_layer(i, assets_path)
