...
'''
Source: https://github.com/rhys-m-adams/Smooth-isotonic-convex-regressor/blob/main/isotonic_regressor.py
Example: https://github.com/rhys-m-adams/Smooth-isotonic-convex-regressor/blob/main/smooth_isotonic_regressor.ipynb
'''
class isotonic_regressor:
    ...
    def spline_penalty(self, grid_points):
        ...
        W = np.diag(np.ones(n - 3) * delta_b[:-2] / 6, k=-1) + np.diag(np.ones(n - 3) * delta_b[2:] / 6, k=1) + np.diag(
            np.ones(n - 2) * (delta_b[:-1] + delta_b[1:]) / 3, k=0)
        delta = np.array(
            diags([1. / delta_b[:-1], -1. / delta_b[:-1] - 1. / delta_b[1:], 1. / delta_b[1:]], offsets=[0, 1, 2],
                  shape=(n - 2, n)).todense())
    ...
    def get_curve(self, x, y, alpha=0, min_x=None, max_x=None, init_vals=None, P=None, q=None, G=None, h=None,
                  x_grid=None):
        ...
        y_expand = np.hstack(
            (fit_y, fit_y[-1] + slope, fit_y[-1] + 2 * slope, fit_y[-1] + 3 * slope, fit_y[-1] + 1e14 * slope))
    ...
    def fit_alphas(self, xs, ys, alphas, min_x=None, max_x=None):
        ...
        for alpha, P in zip(alphas[::-1], Ps[::-1]):
            ...
            try:
                x_grid, fit_y, fx, dfdx, df2dx2 = self.get_curve(xs, ys, min_x=min_x, max_x=max_x, init_vals=fit_fx,
                                                                 P=P, q=q, G=G, h=h, x_grid=x_grid)
                LL = np.array(P).dot((fit_y - mu) / s).dot((fit_y - mu) / s) - 2 * np.array(q).flatten().dot(
                    (fit_y - mu) / s)  # + ys.dot(ys)
